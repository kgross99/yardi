...
void simpleFreeTest()
{
    ...
    if (x == NULL) {
	fprintf(stderr,
		"ERROR! Buddy system failed to allocate 1 byte.\n");
	exit(1);
    }
    if (verbosity > 0) {
...
void backAndForthFreeTest()
{
    void *x;
    void *y;
    void *z;
    printf("running back and forth free test");
    // buddy_malloc one byte to make buddy system
    // split all the way down
    x = buddy_malloc(150);
    y = buddy_malloc(250);
    z = buddy_malloc(100000);
    buddy_free(x);
    ...
    buddy_free(z);
    z = buddy_malloc(100000);
    y = buddy_malloc(200000);
    x = buddy_malloc(500);
    buddy_free(x);
    x = buddy_malloc(2304);
    buddy_free(x);
    printBuddyNode(29);		//Should be Not Null;
    buddy_free(z);
    buddy_free(y);
    printBuddyNode(28);
}
...
void maxAllocationTest()
{
	...
	if (ptr == NULL) {
	    ...
	    if (verbosity > 0) {
		printf
		    ("ERROR! buddy_malloc'd failed to allocate a block of size %lu\n",
		     count);
	    }
	    if (verbosity > 1) {
		printAndClear();
